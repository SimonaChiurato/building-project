...
class EplusEnv(Env):
    ...
    def __init__(self, eplus_path, weather_path, bcvtb_path, variable_path, idf_path, env_name, 
        ...
        (self._eplus_version,self._eplus_run_st_mon, self._eplus_run_st_day,
    ...
    def reset(self):
        ...
        rcv_1st = conn.recv(20*2048).decode(encoding = 'ISO-8859-1');
    ...
    def step(self, action):
        ...
        # print(curSimTim,Dblist,is_terminal)
        ret.append(curSimTim);
        ret.append(Dblist);
        # Add terminal status
        ret.append(is_terminal);
    ...
    def _create_eplus(self, eplus_path, weather_path, 
        ...
        eplus_process = subprocess.Popen('%s -w %s --readvars -j 6 -d %s %s'
    ...
    def _get_eplus_run_info(self, idf_path):
        ...
        for i in range(len(contents)):
            ...
            effectiveContent = effectiveContent.strip().split(',')[0]
                                                          # Remove tailing ','
            if effectiveContent.lower() == 'version':

                epv = (line.strip().split('!')[0].strip().split(',')[1].split(';')[0])
                if epv[0].isdigit():
                    ret.append(epv)
                else:
                    raise Exception('Check version line in idf file',line,epv[0])
        ...
        try:
            ...
            print('Check runperiod Day of Week')
        # Step size
        line_count = 0;
